...
def run_daemon(target_dir: Path, mode: str):
    ...
    watcher = FolderWatcher(target_dir, mode=mode)
    if watcher.start():
        try:
            while True:
                time.sleep(1)
        except KeyboardInterrupt:
            print("\nArrêt du daemon par l'utilisateur.")
            watcher.stop()
    else:
        print("❌ Échec du démarrage du service de surveillance.")

def main():
    parser = argparse.ArgumentParser(description="Smart File Organizer CLI - Tri chirurgical Open-Source, IA DeepSeek & Service d'arrière-plan")
    
    subparsers = parser.add_subparsers(dest="command", help="Commandes disponibles")

    # Command: scan
    scan_parser = subparsers.add_parser("scan", help="Scanner et afficher les propositions de tri (Dry-Run)")
    scan_parser.add_argument("-d", "--dir", type=str, default="DOWNLOADS", help="Dossier cible")
    scan_parser.add_argument("-m", "--mode", choices=["type", "date", "size", "ai"], default="type", help="Mode de tri")
    scan_parser.add_argument("--ai", action="store_true", help="Activer le mode IA DeepSeek")
    scan_parser.add_argument("--regex", type=str, help="Filtre Regex sur le nom des fichiers")
    scan_parser.add_argument("--min-size", type=float, help="Taille min en Mo")
    scan_parser.add_argument("--max-size", type=float, help="Taille max en Mo")
    scan_parser.add_argument("--days", type=int, help="Fichiers modifiés au cours des X derniers jours")

    # Command: organize
    org_parser = subparsers.add_parser("organize", help="Scanner et exécuter le tri de fichiers")
    org_parser.add_argument("-d", "--dir", type=str, default="DOWNLOADS", help="Dossier cible")
    org_parser.add_argument("-m", "--mode", choices=["type", "date", "size", "ai"], default="type", help="Mode de tri")
    org_parser.add_argument("--ai", action="store_true", help="Activer le mode IA DeepSeek")
    org_parser.add_argument("-y", "--yes", action="store_true", help="Confirmer automatiquement sans prompt")
    org_parser.add_argument("--regex", type=str, help="Filtre Regex sur le nom des fichiers")
    org_parser.add_argument("--min-size", type=float, help="Taille min en Mo")
    org_parser.add_argument("--max-size", type=float, help="Taille max en Mo")
    org_parser.add_argument("--days", type=int, help="Fichiers modifiés au cours des X derniers jours")

    # Command: daemon
    daemon_parser = subparsers.add_parser("daemon", help="Lancer la surveillance continue en arrière-plan")
    daemon_parser.add_argument("-d", "--dir", type=str, default="DOWNLOADS", help="Dossier cible")
    daemon_parser.add_argument("-m", "--mode", choices=["type", "date", "size", "ai"], default="type", help="Mode de tri")
    daemon_parser.add_argument("--ai", action="store_true", help="Activer le mode IA DeepSeek pour le daemon")
